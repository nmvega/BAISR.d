...
class GraphBuilder:
    ...
    def create_graph_data(self, data, sizing_mode='combined', 
        ...
        if show_hierarchical:
            hier_count = 0
            for _, comp in components_df.iterrows():
                # Note: Our schema doesn't have parent_component_id - skipping hierarchical edges
                if False:  # comp.get('parent_component_id'):
                    edge = self._create_hierarchical_edge(comp)
                    edges.append(edge)
                    hier_count += 1
            logger.debug(f"Created {hier_count} hierarchical edges")
        
        # All relationships (including dependencies and peer)
        if relationships_df is not None:
            ...
            for _, rel in relationships_df.iterrows():
                ...
                if show_dependencies and rel_type in ('persists_to', 'consumes_api_from', 
                                                       'subscribes_to', 'publishes_to', 'caches_from'):
                    edge = self._create_dependency_edge(rel)
                    edges.append(edge)
                    dep_count += 1
                # Otherwise treat as peer relationship
                elif show_peer:

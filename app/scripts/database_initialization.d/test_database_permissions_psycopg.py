#!/usr/bin/env python3
"""
Database Permission Test Script V2 (psycopg3 version with pglast)
==================================================================
...
class DatabasePermissionTester:
    """Introspective permission tester using psycopg3 and SQL schema parsing"""
    ...
    def check_permission(self, conn, user: str, schema: str, table: str, privilege: str) -> bool:
        """Check if user has specific privilege on table using PostgreSQL introspection"""
        with conn.cursor() as cur:
            # Use PostgreSQL's built-in privilege checking
            sql = f"SELECT has_table_privilege(%s, %s, %s)"
            cur.execute(sql, (user, f"{schema}.{table}", privilege))
            result = cur.fetchone()
            return result[0] if result else False
    
    def check_column_permission(self, conn, user: str, schema: str, table: str, column: str, privilege: str) -> bool:
        """Check if user has specific privilege on a column"""
        with conn.cursor() as cur:
            sql = f"SELECT has_column_privilege(%s, %s, %s, %s)"
            cur.execute(sql, (user, f"{schema}.{table}", column, privilege))
            result = cur.fetchone()
            return result[0] if result else False
    
    def get_tables_in_schema(self, conn, schema: str) -> List[str]:
        """Get all tables in a schema (for verification)"""
        with conn.cursor() as cur:
            sql = """
                SELECT tablename 
                FROM pg_tables 
    ...
    def test_special_permissions(self, conn):
        ...
        for col_grant in self.schema_info['column_grants']:
            if (col_grant['user'] == 'bais_rw' and 
                ...
                if self.check_column_permission(conn, 'bais_rw', 'live', 'auth_users', 'last_login', 'UPDATE'):
                ...
                test_columns = [col for col in all_columns 
                              if col not in ['last_login', 'user_id', 'created_at', 'updated_at']][:3]  # Test first 3
                
                for col in test_columns:
                    if not self.check_column_permission(conn, 'bais_rw', 'live', 'auth_users', col, 'UPDATE'):
    ...
    def test_column_level_permissions(self, conn):
        ...
        for col in test_columns:
            # Test INSERT permission on column
            if self.check_column_permission(conn, 'bais_rw', 'demo', 'biz_components', col, 'INSERT'):
        ...
        for col in test_columns:
            ...
            if self.check_column_permission(conn, 'bais_ro', 'demo', 'biz_components', col, 'SELECT'):
            ...
            # Test INSERT permission - should fail (bais_ro should NOT have INSERT)
            if not self.check_column_permission(conn, 'bais_ro', 'demo', 'biz_components', col, 'INSERT'):
    ...
    def test_fk_constraints(self, conn):
        ...
        if fk_columns:
            # Get column nullable status from information schema
            with conn.cursor() as cur:
                column_names = tuple(col['name'] for col in fk_columns)
    ...
    def run_tests(self):
        ...
        print("=" * 70)
        print(" " * 15 + "DATABASE PERMISSION TEST V3 (psycopg3)")
        print("=" * 70)
        ...
        try:
            with psycopg.connect(**self.conn_params) as conn:
                ...
                for schema in schemas:
                    ...
                    # Verify schema exists and get actual tables
                    actual_tables = set(self.get_tables_in_schema(conn, schema))
                    ...
                    for test_user in ['bais_ro', 'bais_rw']:
                        ...
                        for rule in user_rules:
                            ...
                            # SELECT
                            has_select = self.check_permission(conn, test_user, schema, rule.table, 'SELECT')
                            if has_select != rule.select:
                                failures.append('SELECT')
                                
                            # INSERT
                            has_insert = self.check_permission(conn, test_user, schema, rule.table, 'INSERT')
                            if has_insert != rule.insert:
                                failures.append('INSERT')
                                
                            # UPDATE
                            has_update = self.check_permission(conn, test_user, schema, rule.table, 'UPDATE')
                            if has_update != rule.update:
                                failures.append('UPDATE')
                                
                            # DELETE
                            has_delete = self.check_permission(conn, test_user, schema, rule.table, 'DELETE')
                ...
                self.test_special_permissions(conn)
                
                # Test column-level permissions
                self.test_column_level_permissions(conn)
                
                # Test FK constraints
                self.test_fk_constraints(conn)
